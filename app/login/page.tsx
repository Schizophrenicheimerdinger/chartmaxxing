    ...
    <div style={{ minHeight: '100vh', background: BG, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'Inter, system-ui, sans-serif' }}>
      <div style={{ width: 380, background: SURFACE, border: `1px solid ${BORDER}`, borderRadius: 16, padding: 36 }}>
        ...
        <p style={{ color: MUTED, fontSize: 14, margin: '0 0 28px' }}>
          {isForgot ? "We'll send a reset link to your email" : isSignUp ? 'Sign up to get started' : 'Sign in to your account'}
        </p>
        ...
        {!isForgot && <input value={password} onChange={e => setPassword(e.target.value)} placeholder="Password" type="password" style={input} />}
        ...
        {!isForgot && !isSignUp && (
          <div style={{ textAlign: 'center', marginBottom: 12 }}>
            <button onClick={() => { setIsForgot(true); setError(''); setMessage('') }}
        ...
        <div style={{ textAlign: 'center', fontSize: 13, color: MUTED }}>
          {isForgot ? 'Remember it? ' : isSignUp ? 'Already have an account? ' : "Don't have an account? "}
          <button onClick={() => { setIsForgot(false); setIsSignUp(isForgot ? false : !isSignUp); setError(''); setMessage('') }}
