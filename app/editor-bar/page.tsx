...
import { drawBarChart, type BarChartStyle, BAR_PRESETS, BAR_FONTS } from '@/lib/bar-chart'
...
function EditorBarInner() {
  ...
  const [style, setStyle] = useState<BarChartStyle>(BAR_PRESETS.default.style)
  ...
  const saveProject = useCallback(() => {
    ...
    fetch(`/api/projects/${projectId}`, {
      ...
      body: JSON.stringify({
        ...
        settings: { style, ratio, showValues, showGlow, speed }
      })
    }).then(() => setSaveStatus('saved'))
  }, [projectId, loaded, projectTitle, rows, title, subtitle, yLabel, style, ratio, showValues, showGlow, speed])
  ...
  }, [rows, title, subtitle, yLabel, style, ratio, showValues, showGlow, speed, projectTitle])
  ...
  const chartProps = { data, title, subtitle, yLabel, showValues, ratio, style: effectiveStyle }

  const redraw = useCallback((progress = 1) => {
    if (canvasRef.current) drawBarChart(canvasRef.current, progress, chartProps)
  }, [chartProps])
  ...
  const updateStyle = (key: keyof BarChartStyle, val: string | number) =>
  ...
  if (isMobile) {
      ...
      <>
        ...
        {rightTab === 'colors' && (
          <div>
            ...
            <div style={{ height: 1, background: BORDER, margin: '16px 0' }} />
            <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: 1.2, textTransform: 'uppercase', color: MUTED, marginBottom: 10 }}>Bars</div>
            <ColorRow label="Bar top" value={style.barColor} onChange={v => updateStyle('barColor', v)} />
            <ColorRow label="Bar bottom" value={style.barColor2} onChange={v => updateStyle('barColor2', v)} />
    ...
    return (
      <>
        ...
        {showImport && <DataImportModal chartType="bar" onImport={rows => setRows(rows as any)} onClose={() => setShowImport(false)} />}
        {upgradePrompt}
        <MobileEditorShell
...
export default function EditorBar() {
  return <Suspense fallback={<div style={{ background: '#0c0c10', height: '100vh' }} />}><EditorBarInner /></Suspense>
}